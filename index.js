...
const vcard = 'BEGIN:VCARD\n'
            ...
            + 'FN:Ownerbot\n'
            + 'ORG:Creator SAD BOT;\n'
            + 'TEL;type=CELL;type=VOICE;waid=628532079751:+62 858-3207-9751\n'
            + 'END:VCARD'
prefix = '!'
blocked = []            
...
hafizh.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `@${num.split('@')[0]}\nwelcome to group *${mdata.subject}* semoga ngak betah`
	...
	hafizh.on('message-new', async (tod) => {
		try {
			...
			const ownerNumber = ["6285832079751@s.whatsapp.net"] // ganti nomer lu
